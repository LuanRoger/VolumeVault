...
class BookService {
  ...
  Future<HttpResponse> getUserBook(
      GetUserBookRequest getUserBookRequest) async {
    HttpResponse response =
        await _httpModule.get(_baseUrl, query: getUserBookRequest.forRequest());

    //TODO: Chek response returned infos
    dynamic booksResponse = json.decode(response.body);
    List<BookModel> books = List.empty(growable: true);
    for (Map<String, dynamic> jsonBook in booksResponse) {
